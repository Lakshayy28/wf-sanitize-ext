...
exports.HIGH_CONFIDENCE_SECRETS = exports.MASK = exports.SENSITIVE_SUFFIXES = exports.DYNAMIC_AST_KEYS = void 0;
...
// ────────────────────────────────────────────────────────────────────────────
exports.DYNAMIC_AST_KEYS = new Set([
 ...
 */
exports.SENSITIVE_SUFFIXES = new Set([
...
function isSensitiveKey(key) {
    ...
    if (tokens.some(t => exports.DYNAMIC_AST_KEYS.has(t))) {
        return true;
    }
    const lastMeaningful = [...tokens].reverse().find(t => !/^\d+$/.test(t) && t.length > 1);
    return lastMeaningful !== undefined && exports.SENSITIVE_SUFFIXES.has(lastMeaningful);
}
// ────────────────────────────────────────────────────────────────────────────
// Mask placeholder
// ────────────────────────────────────────────────────────────────────────────
 ...
 */
function hydrateCustomSecrets(customSecrets) {
    for (const def of customSecrets) {
        // Inject AST keys
        if (def.ast_keys) {
            for (const k of def.ast_keys) {
                exports.DYNAMIC_AST_KEYS.add(k.toLowerCase());
            }
        }
        ...
        try {
            ...
            exports.HIGH_CONFIDENCE_SECRETS.unshift({
                name: def.name,
                regex: compiledRegex,
            });
        }
...
function regexSanitize(text) {
    ...
    for (const rule of exports.HIGH_CONFIDENCE_SECRETS) {
        rule.regex.lastIndex = 0;
        if (rule.isUrlAuth) {
            // Special handler for URLs: Rebuilds as protocol + MASK + @host
            cleanText = cleanText.replace(rule.regex, (_match, p1, _p2, p3) => {
                modified = true;
                return `${p1}${exports.MASK}${p3}`;
            });
        }
        else {
...
function terminalSanitize(text) {
    ...
    result = result.replace(/(--(?:password|passwd|token|secret|api[_-]?key|auth[_-]?token|access[_-]?token)\s*[=\s]\s*)(\S+)/gi, (_, prefix) => { wasModified = true; return prefix + exports.MASK; });
    // curl -H "Authorization: Bearer <token>"
    result = result.replace(/(-H\s+["'](?:Authorization|X-Api-Key|X-Auth-Token)\s*:\s*(?:Bearer\s+)?)([^"']+)(["'])/gi, (_, prefix, _val, quote) => { wasModified = true; return prefix + exports.MASK + quote; });
    // JSON fields: "password": "value"
    result = result.replace(/(["'](?:password|passwd|secret|api_?key|token|access_?token|auth_?token|private_?key|client_?secret|jwt_?secret|session_?secret|signing_?key|encryption_?key|database_?url|connection_?string)["']\s*:\s*["'])([^"']+)(["'])/gi, (_, prefix, _val, quote) => { wasModified = true; return prefix + exports.MASK + quote; });
    // Env var assignments
    result = result.replace(/((?:^|\n)\s*(?:export\s+|SET\s+|ENV\s+)?(?:PASSWORD|PASSWD|SECRET|API_?KEY|TOKEN|ACCESS_?TOKEN|AUTH_?TOKEN|PRIVATE_?KEY|CLIENT_?SECRET|AWS_?SECRET_?ACCESS_?KEY|AWS_?SESSION_?TOKEN|DATABASE_?URL|DB_?PASSWORD|REDIS_?PASSWORD|MONGO_?URI)=)(.+)/gim, (_, prefix) => { wasModified = true; return prefix + exports.MASK; });
    // Bearer tokens (standalone)
    result = result.replace(/(Bearer\s+)([A-Za-z0-9\-._~+/]+=*)/gi, (_, prefix) => { wasModified = true; return prefix + exports.MASK; });
    // AWS Access Key IDs
    result = result.replace(/(?:AKIA|ASIA)[0-9A-Z]{16}/g, () => { wasModified = true; return exports.MASK; });
