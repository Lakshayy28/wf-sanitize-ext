...
exports.BUILTIN_PII_PATTERN_NAMES = exports.PII_KEYS = void 0;
exports.hydratePiiConfig = hydratePiiConfig;
exports.localPiiScan = localPiiScan;
const regexSanitizer_1 = require("./regexSanitizer");
// ────────────────────────────────────────────────────────────────────────────
// PII Key Dictionary — labels that indicate the VALUE contains personal data
// ────────────────────────────────────────────────────────────────────────────
exports.PII_KEYS = new Set([
    // Identity
...
// ────────────────────────────────────────────────────────────────────────────
const BUILTIN_PII_VALUE_PATTERNS = [
...
exports.BUILTIN_PII_PATTERN_NAMES = BUILTIN_PII_VALUE_PATTERNS.map(p => p.name);
// ────────────────────────────────────────────────────────────────────────────
// Runtime-active patterns (rebuilt by hydratePiiConfig on each config load)
// ────────────────────────────────────────────────────────────────────────────
let activePiiPatterns = [...BUILTIN_PII_VALUE_PATTERNS];
// ────────────────────────────────────────────────────────────────────────────
 ...
 */
function hydratePiiConfig(config) {
    ...
    for (const key of config?.extra_keys ?? []) {
        regexSanitizer_1.DYNAMIC_AST_KEYS.add(key.toLowerCase());
        exports.PII_KEYS.add(key.toLowerCase());
    }
}
// ────────────────────────────────────────────────────────────────────────────
// Public API
 ...
 */
function localPiiScan(value) {
    let result = value;
    for (const { regex } of activePiiPatterns) {
        regex.lastIndex = 0;
        result = result.replace(regex, regexSanitizer_1.MASK);
    }
    return result;
}
